...
/*
 ...
 | I changed CRM_Case_XMLProcessor_Report to CRM_Dgwcasetopdf_Case_XMLProcessor_Report, and  |
 | copy the function printCaseReport to htmlCaseReport (with a couple of changes)         |
 +----------------------------------------------------------------------------------------+
*/
class CRM_Dgwcasetopdf_Case_XMLProcessor_Report extends CRM_Case_XMLProcessor {
  ...
  function getActivities($clientID, $caseID, $activityTypes, &$activities) {
    ...
    $activitytypeLetopgevoeligeInformatieid = $configDgwCaseToPdf->activityTypeGevoeligeDossierinformatieId;
    unset($coreActivityTypes[$activitytypeLetopgevoeligeInformatieid]);
...
";
  ...
  function printCaseReport() {
    ...
    $report            = new CRM_Dgwcasetopdf_Case_XMLProcessor_Report($isRedact);
  ...
  function htmlCaseReport($caseID, $clientID, $activitySetName = 'standard_timeline', $isRedact = 0, $includeActivities = 1) {
    ...
    $configDgwCaseToPdf = CRM_Dgwcasetopdf_Config::singleton();
    $activitytypeLetopgevoeligeInformatieid = $configDgwCaseToPdf->activityTypeGevoeligeDossierinformatieId;
    $template->assign('hideActivityTypes', array($activitytypeLetopgevoeligeInformatieid));
    ...
    $printReport = CRM_Case_Audit_Audit::run($contents, $clientID, $caseID, TRUE, array($activitytypeLetopgevoeligeInformatieid));
    return $printReport;
    //CRM_Utils_System::civiExit();
  }
}
