...
function civicrm_api3_dgw_case_to_pdf_dgwcasetopdfbeforehovenddate($params) {
  ...
  while ($dao->fetch()) { 
    ...
    */
    ...
    if(CRM_Casetopdf_Config::file_exists($pathname . 'too_big_' . implode('_', $pathvar) . '.txt')){
      $return['message'][] = ts('File \'%1\' already exist, was too big !', array(1 => $pathname . 'too_big_' . implode('_', $pathvar) . '.txt'));
      if($debug){
        echo ts('File \'%1\' already exist, was too big !', array(1 => $pathname . 'too_big_' . implode('_', $pathvar) . '.txt')) . '<br/>' . PHP_EOL;
      }
      continue;
    }
    ...
    if($big and 82710 <= strlen($html)){
      ...
      $_return = CRM_Casetopdf_Config::fwrite($pathname . 'too_big_' . implode('_', $pathvar) . '.txt', 'Html is too big to convert to pdf !', 'w');
      if($_return['is_error']){
